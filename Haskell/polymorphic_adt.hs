...
lzipWith1 op (Cons x1 xs1) (Cons x2 xs2) =
  Cons (op x1 x2) (lzipWith1 op xs1 xs2)
...
lzipWith2 op (Cons x1 xs1) (Cons x2 xs2) =
  Cons (op x1 x2) (lzipWith1 op xs1 xs2)

-- using foldr
zw op l1 l2 =
  let len = lsize l1 - 1
   in lfoldr (\f acc -> Cons (f (at (len - lsize acc) l2)) acc) Empty l1'
  where
    l1' = lmap (\x -> op x) l1
...
tfoldl op acc (Node key left right) =
  let res = (tfoldl op acc left)
   in tfoldl op (op key res) right

-- accumulates starting from left
tfoldr :: (t -> t -> t) -> t -> (Tree t) -> t
tfoldr _ acc Null = acc
tfoldr op acc (Leaf a) = op acc a
tfoldr op acc (Node key left right) =
  let res = (tfoldr op acc right)
   in tfoldr op (op key res) left

-- accumulates going down and propagating up
tfold :: (t -> t -> t) -> t -> (Tree t) -> t
tfold _ acc Null = acc
tfold op acc (Leaf a) = op acc a
tfold op acc (Node key left right) =
  let rest = op onLeft onRight
   in op key rest
  where
    onLeft = tfold op acc left
...
tzipWith op (Node k1 l1 r1) (Node k2 l2 r2) =
  Node (op k1 k2) (tzipWith op l1 l2) (tzipWith op r1 r2)
tzipWith _ _ _ = Null
