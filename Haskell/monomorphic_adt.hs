...
data BBool = BTrue | BFalse deriving Show
...
data Formula = Var String | And Formula Formula | Or Formula Formula | Not Formula deriving Show
...
data Tree = Null | Node Integer Tree Tree | Leaf Integer deriving Show
...
foldTree op acc (Node key left right) = let val = foldTree op acc left in foldTree op (op key val) right
